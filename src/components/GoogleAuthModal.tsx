...
export const GoogleAuthModal: React.FC<GoogleAuthModalProps> = ({ isOpen, onClose }) => {
  ...
  const [isSigningInFirebase, setIsSigningInFirebase] = useState(false);
  const [authError, setAuthError] = useState<string | null>(null);

  if (!isOpen) return null;

  const handleFirebaseSignIn = async () => {
    setIsSigningInFirebase(true);
    setAuthError(null);
    try {
      ...
      setAuthError(err.message || 'Firebase Google Sign-in was cancelled or popup was blocked.');
    } finally {
      setIsSigningInFirebase(false);
    }
  };
      ...
      >
        ...
        <div className="bg-slate-900 p-6 text-white relative">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-300">Firebase Auth & persistent Firestore cloud profile synchronization</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-6 pb-2">
          ...
          >
            ...
            <span>{isSigningInFirebase ? 'Connecting to Google...' : 'Sign in with Google (Firebase Auth)'}</span>
          </button>
